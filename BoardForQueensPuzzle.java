...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
    	if (lastRankFilled == -1 || lastRankFilled == 0) return false;
    	int lastFile = filesWithQueens[lastRankFilled];
    	for (int i = 0; i < lastRankFilled; i++) {
    		if (filesWithQueens[i] == lastFile) {
    			return true;
    		}
    		if (Math.abs(lastRankFilled - i) == Math.abs(lastFile - filesWithQueens[i])) {
    			return true;
    		}
    	}
    	return false;
    ...
    public boolean accept() {
    	if (lastRankFilled != this.ranks() - 1) return false;
    	if(this.lastIsNg()) return false;
    	return true;
    }
